...
class Register(Resource):
    ...
    def post(self):
        if not re.search(USERNAME_REGEXP, ns.payload['username']):
            ns.abort(400, 'Username should have 4-16 symbols, can contain A-Z, a-z, 0-9, _ (_ can not be at the begin/end and can not go in a row (__))')
...
class Login(Resource):
    ...
    def post(self):
        ...
        if check_password_hash(user.passwordHash, ns.payload['password']):
            accessPayload = {
                'uid': user.id,
                'exp': datetime.datetime.utcnow() + datetime.timedelta(seconds=current_app.config['ACCESS_EXP_SECS']),
            ...
            refreshPayload = {
                'uid': user.id,
                'exp': datetime.datetime.utcnow() + datetime.timedelta(seconds=current_app.config['REFRESH_EXP_SECS']),
                'iat': datetime.datetime.utcnow()
            }
        ...
        if authHeader:
            try:
                ...
                try:
                    ...
                    currentUser = User.query.get(token['uid'])
